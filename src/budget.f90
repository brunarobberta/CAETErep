...
contains
   ...
   subroutine daily_budget(dt, w1, w2, ts, temp, prec, n_days, p0, ipar, rh&
      ...
      real(r_8),dimension(:),allocatable :: seed_bank !NEW ***************
      real(r_8),dimension(:),allocatable :: n_seed    !NEW ***************
      real(r_8),dimension(:),allocatable :: germinated_seeds    !NEW ***************
      real(r_8),dimension(:),allocatable :: total_C_pls    !NEW ***************
      real(r_8),dimension(:),allocatable :: pct_leaf_C    !NEW ***************
      real(r_8),dimension(:),allocatable :: pct_wood_C    !NEW ***************
      real(r_8),dimension(:),allocatable :: pct_froot_C    !NEW ***************
      ...
      do p = 1,nlen
         ...
         if (24.0 <= temp .and. temp <= 33.0 .and. 60.0 <= prec .and. prec <= 200.0 .and. nppa(p) > 0.0) then
            ...
            call repro(nppa(p), height_aux(ri), seed_mass(ri), n_seed(ri), seed_bank(ri)) ! ---> Usar height_aux(ri) ou height_aux(p) ???
            seed_bank(ri) = seed_bank(ri) + n_seed(ri) !!UPDATE SEEDBANK

            ! Garantir que seed_bank não seja negativo após a produção
